...
| [Building hybrid search on Couchbase](notebooks/retrieval/01_building_hybrid_search.ipynb) | retrieval | **Start here for the mechanics.** Vector, then keyword, then both in one request, then filtered — each step a working search over 6,482 real products. Analysers, prefilters, one index. | Two queries, opposite winners — which is why 02 exists |
| [Which parts of that were worth it?](notebooks/retrieval/02_which_parts_helped.ipynb) | retrieval | Takes the four searches from 01 and measures them against 7,000 human relevance judgements with `trec_eval`. No LLM, no cost. | No winner: vector leads nDCG@10 (0.78 vs 0.71), the hybrids lead recall@50 |
| [Documents that learn](notebooks/data-model/01_documents_that_learn.ipynb) | data-model | Twenty contracts arrive as prose. A model reads their terms; the terms go back onto the documents — embedded, and again as referenced derived documents — and SQL++ queries them. No migration, no second store. | Prose becomes `WHERE governing_law = "California" AND agreement_date > "2015"` |
| [Adding retrieval to enriched documents](notebooks/data-model/02_adding_retrieval.ipynb) | data-model | Chunks are derived documents. A search index cannot join them to their parent — so copy the filterable fields down, or filter afterwards and lose results. Both, measured. | A filter on an unindexed field returns 0 hits and no error |
